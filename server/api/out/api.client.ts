import { AppError, ErrorCode } from "@core/shared/errors";
...
export class ApiClient {
  ...
  async get<T = any>(endpoint: string): Promise<T> {
    try {
      ...
      throw new AppError(
        "NETWORK_ERROR",
        `Network error calling GET ${endpoint}`,
        "external",
        {
          endpoint,
          method: "GET",
  ...
  async post<T = any>(endpoint: string, body: any): Promise<T> {
    try {
      ...
      throw new AppError(
        "NETWORK_ERROR",
        `Network error calling POST ${endpoint}`,
        "external",
        {
          endpoint,
          method: "POST",
  ...
  async put<T = any>(endpoint: string, body: any): Promise<T> {
    try {
      ...
      throw new AppError(
        "NETWORK_ERROR",
        `Network error calling PUT ${endpoint}`,
        "external",
        {
          endpoint,
          method: "PUT",
