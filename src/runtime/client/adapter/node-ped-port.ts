...
export class NodeClientPedPort extends IClientPedPort {
  async spawn(_options: ClientPedSpawnOptions): Promise<number> { return 0 }
  delete(_handle: number): void {}
  exists(_handle: number): boolean { return false }
  ...
  giveWeapon(_handle: number, _weapon: string, _ammo = 100, _hidden = false, _forceInHand = true): void {}
  removeAllWeapons(_handle: number): void {}
  getClosest(_radius = 10, _excludeLocalPlayer = true): number | null { return null }
  getNearby(_position: Vector3, _radius: number, _excludeEntity?: number): number[] { return [] }
