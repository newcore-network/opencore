...
import { IClientBlipBridge, type ClientBlipOptions } from '../../../adapters/contracts/client/ui/IClientBlipBridge'
...
export class PlatformBlipBridge extends IClientBlipBridge {
  ...
  create(position: Vector3): number { return this.platform.addBlipForCoord(position) }
  createForEntity(entity: number): number { return this.platform.addBlipForEntity(entity) }
  createForRadius(position: Vector3, radius: number): number { return this.platform.addBlipForRadius(position, radius) }
  exists(handle: number): boolean { return this.platform.doesBlipExist(handle) }
  remove(handle: number): void { this.platform.removeBlip(handle) }
  setPosition(handle: number, position: Vector3): void { this.platform.setBlipCoords(handle, position) }
  applyOptions(handle: number, options: ClientBlipOptions): void {
    ...
    if (options.shortRange !== undefined) this.platform.setBlipAsShortRange(handle, options.shortRange)
    ...
    if (options.routeColor !== undefined) this.platform.setBlipRouteColour(handle, options.routeColor)
  }
}
