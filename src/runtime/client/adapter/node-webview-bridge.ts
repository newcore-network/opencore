...
export class NodeClientWebViewBridge extends IClientWebViewBridge {
  ...
  getCapabilities(): WebViewCapabilities { return NODE_WEBVIEW_CAPABILITIES }
  create(definition: WebViewDefinition): void { this.views.add(definition.id) }
  destroy(viewId: string): void { this.views.delete(viewId) }
  ...
  onMessage(_handler: (message: WebViewMessage) => void | Promise<void>): () => void { return () => {} }
}
