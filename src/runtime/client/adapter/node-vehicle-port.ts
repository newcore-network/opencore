...
export class NodeClientVehiclePort extends IClientVehiclePort {
  ...
  getFuel(_vehicle: number): number { return 0 }
  getClosest(_radius = 10): number | null { return null }
  isLocalPlayerInVehicle(): boolean { return false }
  getCurrentForLocalPlayer(): number | null { return null }
  getLastForLocalPlayer(): number | null { return null }
  isLocalPlayerDriver(_vehicle: number): boolean { return false }
  ...
  getSpeed(_vehicle: number): number { return 0 }
  setHeading(_vehicle: number, _heading: number): void {}
  teleport(_vehicle: number, _position: Vector3, _heading?: number): void {}
  exists(_vehicle: number): boolean { return false }
  getNetworkId(_vehicle: number): number { return 0 }
  getFromNetworkId(_networkId: number): number { return 0 }
  getState<T = unknown>(_vehicle: number, _key: string): T | undefined { return undefined }
  getPosition(_vehicle: number): Vector3 | null { return null }
  getHeading(_vehicle: number): number { return 0 }
  getModel(_vehicle: number): number { return 0 }
  getPlate(_vehicle: number): string { return '' }
}
