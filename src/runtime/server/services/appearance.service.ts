...
export class AppearanceService {
  validateAppearance(appearance: Partial<PlayerAppearance>): AppearanceValidationResult {
    ...
    if (appearance.headBlend) {
      ...
      if (typeof hb.shapeFirst !== 'number' || hb.shapeFirst < 0 || hb.shapeFirst > 45) errors.push('Invalid shapeFirst (must be 0-45)')
      if (typeof hb.shapeSecond !== 'number' || hb.shapeSecond < 0 || hb.shapeSecond > 45) errors.push('Invalid shapeSecond (must be 0-45)')
      if (typeof hb.skinFirst !== 'number' || hb.skinFirst < 0 || hb.skinFirst > 45) errors.push('Invalid skinFirst (must be 0-45)')
      if (typeof hb.skinSecond !== 'number' || hb.skinSecond < 0 || hb.skinSecond > 45) errors.push('Invalid skinSecond (must be 0-45)')
      if (typeof hb.shapeMix !== 'number' || hb.shapeMix < 0 || hb.shapeMix > 1) errors.push('Invalid shapeMix (must be 0.0-1.0)')
      if (typeof hb.skinMix !== 'number' || hb.skinMix < 0 || hb.skinMix > 1) errors.push('Invalid skinMix (must be 0.0-1.0)')
      if (hb.shapeThird !== undefined && (hb.shapeThird < 0 || hb.shapeThird > 45)) errors.push('Invalid shapeThird (must be 0-45)')
      if (hb.skinThird !== undefined && (hb.skinThird < 0 || hb.skinThird > 45)) errors.push('Invalid skinThird (must be 0-45)')
      if (hb.thirdMix !== undefined && (hb.thirdMix < 0 || hb.thirdMix > 1)) errors.push('Invalid thirdMix (must be 0.0-1.0)')
    }
    ...
    if (appearance.headOverlays) {
      for (const [id, overlay] of Object.entries(appearance.headOverlays)) {
        ...
        if (typeof overlay.index !== 'number' || overlay.index < 0) errors.push(`Invalid overlay index for ID ${id}`)
    ...
    if (appearance.hairColor) {
      if (typeof appearance.hairColor.colorId !== 'number' || appearance.hairColor.colorId < 0) errors.push('Invalid hair colorId')
    ...
    if (appearance.tattoos) {
      if (!Array.isArray(appearance.tattoos)) {
        ...
        for (let i = 0; i < appearance.tattoos.length; i++) {
          ...
          if (!tattoo.collection || typeof tattoo.collection !== 'string') errors.push(`Invalid tattoo collection at index ${i}`)
          if (!tattoo.overlay || typeof tattoo.overlay !== 'string') errors.push(`Invalid tattoo overlay at index ${i}`)
        }
      }
    }
